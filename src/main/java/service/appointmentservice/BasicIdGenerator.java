 ...
 *
 * This class is able to generate unique IDs for any class that extends Appointment passed into generateId(entityClass)
 * - Includes static methods for testing, resetCounters(), setCounter(), and printCounters()
 */
public class BasicIdGenerator
{
    private static final Map<Class<? extends Appointment>, Long> counters = new HashMap<>();
    ...
    public static String generateId(Class<? extends Appointment> entityClass) {
    ...
    public static void setCounter(Class<? extends Appointment> counterClass, Long value) {
        counters.put(counterClass, value);
    }

    // print all counters (Excluded unless needed for debug)
    /*public static void printCounters() {
        for (Map.Entry<Class<? extends Appointment>, Long> mapElement : counters.entrySet()) {
